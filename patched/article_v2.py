...
3) 이 주제의 지식이 어디서 오는지 보고 mode 를 정하세요.

[mode 정하기 — 셋 중 하나]

"source"     자료가 진실의 원천인 주제. 정책·지원금·대출·뉴스·인물.
             금액·기간·조건은 **자료에 있는 것만** 씁니다.
             자료에 없으면 그 질문을 버리세요. 짐작해서 쓰면 안 됩니다.

"knowledge"  널리 확립된 지식이 있는 주제. 건강 기초, 영양, 생활 습관 등.
             교과서나 공공 건강정보 수준으로 굳어진 내용은 자료에 없어도
             쓸 수 있습니다. 오히려 안 쓰면 알맹이 없는 글이 됩니다.
             다만 **약·치료 효과·진단·최신 수치**는 자료에 있는 것만 쓰세요.
             2)에서 버린 질문이라도 이 범위면 되살리세요.

"howto"      지역·기관마다 달라서 하나로 정리할 수 없는 주제.
             → 제도 설명 대신 '내가 해당되는지 확인하는 방법' 글을 씁니다.
             이쪽이 오히려 독자에게 쓸모 있습니다.

남은 질문이 3개 미만이고 knowledge 도 아니면 "howto" 로 하세요.
...
{{
  ...
  "known": ["knowledge 일 때만. 자료에 없지만 널리 확립되어 쓸 수 있는 내용. 아니면 빈 배열"]
}}
""".strip()
...
def parse_questions(raw: str) -> dict[str, Any]:
    ...
    if mode == "info":          # 예전 이름도 받아 준다
        mode = "source"
    if mode not in {"source", "knowledge", "howto"}:
        mode = "source"
    질문들 = [str(하나).strip() for 하나 in (값.get("questions") or []) if str(하나).strip()]
    사실들 = [str(하나).strip() for 하나 in (값.get("facts") or []) if str(하나).strip()]
    아는것 = [str(하나).strip() for 하나 in (값.get("known") or []) if str(하나).strip()]
    # 자료 기반 글인데 답할 질문이 없으면, 억지로 쓰지 말고 방법 글로 돌린다.
    if len(질문들) < 3 and mode == "source":
        mode = "howto"
    return {"mode": mode, "questions": 질문들[:7], "facts": 사실들[:12],
            "known": 아는것[:12], "why": str(값.get("why", "")).strip()}
...
_지식뼈대 = """
[글의 뼈대]
이 주제는 널리 확립된 지식이 있습니다. 자료가 얇다고 얼버무리지 마세요.
교과서·공공 건강정보 수준으로 굳어진 내용은 **구체적인 숫자와 기준으로** 쓰세요.
다만 약·치료 효과·진단·최신 수치는 자료에 있는 것만 씁니다.

- 도입 2~3문단. 결론부터. 독자가 알고 싶은 답을 먼저 주세요.
- 위 질문 하나가 <h2> 하나입니다.
- **각 <h2> 의 첫 문장이 그 질문의 답**입니다.
- "개인차가 있습니다" 로 끝내지 마세요. 개인차가 있다면
  **무엇에 따라 어떻게 달라지는지** 기준을 주세요.
  나쁨: 필요량은 개인차가 있습니다.
  좋음: 체중 1kg당으로 계산합니다. 활동량이 적으면 낮은 쪽, 운동을 하면 높은 쪽입니다.
- 기준·범위·계산법이 있으면 표나 목록으로 만드세요.
- 병원에 가야 하는 선, 조심해야 할 경우를 반드시 한 대목 넣으세요.
- 단정하지 말고, 진단·치료는 의료진 몫이라는 선을 지키세요.
"""
...
def build_article_prompt(
    ...
    아는줄 = ""
    if plan["mode"] == "knowledge" and plan.get("known"):
        아는줄 = "\n[자료에 없지만 널리 확립되어 쓸 수 있는 것]\n" + \
                 "\n".join(f"- {하나}" for 하나 in plan["known"]) + "\n"
    뼈대 = {"howto": _방법뼈대, "knowledge": _지식뼈대}.get(plan["mode"], _정보뼈대)
...
def too_vague(잰것: dict[str, Any], mode: str) -> bool:
    ...
    if 잰것["모호밀도"] > 4.0:
        return True
    # 자료 기반 글과 지식 글은 구체 숫자가 나와야 한다. 없으면 알맹이가 없다.
    # 방법 글(howto)은 절차가 알맹이라 숫자가 없어도 정상이다.
    if mode in {"source", "knowledge"} and 잰것["숫자밀도"] < 0.5 and 잰것["글자수"] > 1500:
        return True
    return False
