...
낱말표: list[tuple[tuple[str, ...], list[str]]] = [
    ...
    # 몸의 기관은 카테고리 이름과 글에서 쓰는 말이 다르다.
    # '지켜낼 신장' 인데 글에는 '콩팥' 이라고 쓴다. 그래서 못 잡았다.
    (("신장", "콩팥", "투석"),
     ["신장", "콩팥", "투석", "사구체", "크레아티닌", "eGFR", "단백뇨", "네프론",
      "만성콩팥병", "신부전", "요독", "혈액투석", "복막투석", "이식"]),
    (("눈", "안과", "시력"),
     ["눈", "시력", "안구", "각막", "망막", "녹내장", "백내장", "안구건조",
      "라식", "라섹", "노안", "결막", "눈물"]),
    (("심장", "혈관", "고혈압"),
     ["심장", "혈관", "혈압", "고혈압", "심근", "부정맥", "콜레스테롤", "동맥"]),
    (("당뇨", "혈당"),
     ["당뇨", "혈당", "인슐린", "당화혈색소", "저혈당", "고혈당"]),
    (("간", "지방간"),
     ["간수치", "지방간", "간염", "간경화", "ALT", "AST", "황달"]),
    (("갑상선", "호르몬"),
     ["갑상선", "호르몬", "갑상샘", "TSH"]),
    (("관절", "허리", "근골격"),
     ["관절", "허리", "디스크", "무릎", "어깨", "골다공증", "연골"]),
    (("정신", "마음", "심리", "멘탈", "스트레스"),
...
# 어느 글에나 붙을 수 있는 넓은 이름. 다른 데가 비었을 때만 쓴다.
넓은이름 = ("건강", "정보", "일반", "기타", "생활", "관련", "이야기", "블로그")
...
def _점수(카테고리이름: str, 글: str) -> int:
    ...
    # ③ '건강미용관련' 처럼 넓은 이름은 깎는다.
    #    안 깎으면 이런 이름이 모든 글을 가져가 버린다. 실제로 콩팥 글이
    #    '지켜낼 신장' 을 두고 '건강미용관련' 로 갔다.
    if 점수 and any(말 in 카테고리이름 for 말 in 넓은이름):
        핵있음 = any(len(_조사떼기(말)) >= 2 and _조사떼기(말) in 글
                     for 말 in _낱말(카테고리이름)
                     if not any(넓 in 말 for 넓 in 넓은이름))
        if not 핵있음:
            점수 = max(0, 점수 - 4)
...
def pick_category_id(
    ...
    if 최고점 <= 0:
        logging.info("어울리는 워드프레스 카테고리를 못 찾아 기본값을 씁니다: %s\n"
                     "  사이트 카테고리: %s",
                     default_category_id,
                     ", ".join(하나["name"] for 하나 in 카테고리들[:10]))
        return default_category_id
