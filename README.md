...
### 두 가지 모드 — A 원본을 어떻게 할 것인가

| 모드 | 명령 | A 원본 | B 새 글 |
|------|------|--------|---------|
| **이관** (기본) | `migrate ... --retire-source` | 비공개 전환 또는 301 리다이렉트 필수 | A 의 업그레이드 버전 (내용 계승) |
| **공존** | `migrate ... --distinct --keyword "다른 키워드"` | 공개 유지 가능 | 같은 주제, **다른 검색 의도·구조의 별개 글** |

기본 모드로 만든 B 글은 A 와 내용이 비슷해서, A 를 공개로 두면 중복 콘텐츠로
둘 다 순위가 깎일 수 있다. A 를 유지하려면 반드시 `--distinct` 를 쓰고,
`--keyword` 로 A 와 다른 롱테일 키워드를 지정하는 것을 권장한다.

이관 후 체크리스트 (명령이 끝나면 자동으로 다시 알려준다):

- **중복 콘텐츠 방지**: 기본 모드면 A 원본 비공개(`--retire-source`) 또는 A→B 301
  리다이렉트. `--distinct` 모드면 두 글의 제목·키워드가 충분히 다른지 확인
