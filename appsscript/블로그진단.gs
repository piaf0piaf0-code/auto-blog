...
function 진단판정(칸) {
  ...
  var 길말 = 칸.다른클릭90 > 0
    ? ' (그중 디스커버·뉴스 ' + 칸.다른클릭90 + (칸.다른클릭90 >= 칸.클릭90 * 0.5 ? ' — 주로 추천 기사로 들어옴. 뉴스성 제목·큰 이미지가 중요' : '') + ')'
    : '';
  if (칸.클릭90 >= 진단집중클릭90 || 수익 >= 진단집중수익) {
    return { 순서: 1, 판정: '집중',
             이유: '90일 클릭 ' + 칸.클릭90 + 길말 + (수익 ? ' · 수익 ' + 수익 + '원' : '') + ' — 구글이 이미 보여 주는 곳. 새 글·보강을 여기에.' + 색인말 };
  }
  if (칸.클릭28 >= 1 || 칸.나온비율 >= 0.1 || 수익 > 0) {
    return { 순서: 2, 판정: '유지 — 손보기',
             이유: '조금씩 나오는 중' + 길말 + '. 새 글보다 SC_할일의 글 보강부터.' + 색인말 };
  }
...
function 블로그진단() {
  ...
  // 디스커버·구글 뉴스로 오는 방문자도 센다. 뉴스 해설 블로그는 이쪽이 더 클 수 있다.
  var 다른28 = SC다른길합계(사이트들, 진단날짜(28));
  var 다른90 = SC다른길합계(사이트들, 진단날짜(90));
  var 다른365 = SC다른길합계(사이트들, 진단날짜(365));
  function 더하기(합, 다른) {
    Object.keys(다른).forEach(function (h) {
      var 칸 = 합[h] || (합[h] = { 클릭: 0, 노출: 0, 글: {} });
      칸.클릭 += 다른[h].합.클릭;
      칸.노출 += 다른[h].합.노출;
      Object.keys(다른[h].글).forEach(function (k) { 칸.글[k] = true; });
    });
  }
  더하기(합28, 다른28); 더하기(합90, 다른90); 더하기(합365, 다른365);
  ...
  Object.keys(호스트표).forEach(function (호스트) {
    ...
    var 칸 = {
      ...
      다른길365: SC다른길글(다른365[호스트]),
      다른클릭90: 다른90[호스트] ? 다른90[호스트].합.클릭 : 0,
      수익: (있던입력[호스트] || {}).수익 || '',
      메모: (있던입력[호스트] || {}).메모 || '',
      색인요약: '', 주된색인: ''
    };
