...
//  사실카드 한 번에 만들기
//
//  주제마다 따로 하면 14번을 해야 한다. 그런데 항목 112개가 실제로는
//  공식 페이지 21곳에서 나온다. 한 페이지에 여러 주제의 값이 같이
//  적혀 있다. 그래서 페이지별로 모아서 한 번에 물어보는 편이 낫다.
//
//  봇이 112줄을 한 번에 못 주면 나눠서 줘도 된다. 저장할 때 받은 줄만
...
function finwiz전체카드요청문() {
  ...
  var 출처표 = {};
  (typeof finwiz출처자료 !== 'undefined' ? finwiz출처자료 : []).forEach(function (한줄) {
    출처표[한줄[0]] = { 기관: 한줄[1], 주제: 한줄[2], URL: 한줄[3], 메모: 한줄[4] || '' };
  });

  var 페이지별 = {}, 순서 = [];
  주제들.forEach(function (주제) {
    주제.항목.forEach(function (한개) {
      var 출처키 = 한개[1] || '기타';
      if (!페이지별[출처키]) { 페이지별[출처키] = []; 순서.push(출처키); }
      페이지별[출처키].push({ 주제: 주제.이름, 항목: 한개[0] });
    });
  });

  var 오늘 = Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'yyyy-MM-dd');
  var 총항목 = 0;
  순서.forEach(function (키) { 총항목 += 페이지별[키].length; });
  var 총글 = 0;
  주제들.forEach(function (하나) { 총글 += 하나.글수; });

  var 조각 = [
    '[하는 일]',
    '블로그 글 ' + 총글 + '편을 쓸 예정입니다. 글마다 숫자를 따로 찾으면 글끼리 숫자가 어긋납니다.',
    '그래서 **지금 한 번만** 공식 사이트에서 확인해 표로 정리해 주세요.',
    '확인할 항목은 모두 ' + 총항목 + '개이고, 공식 페이지 ' + 순서.length + '곳에서 나옵니다.',
    '',
    '[읽는 순서]',
    '아래에 공식 페이지별로 확인할 항목을 모아 두었습니다.',
    '페이지를 하나 열고, 그 페이지에서 확인되는 항목을 다 적은 뒤 다음 페이지로 넘어가 주세요.',
    '한 번에 다 못 주면 나눠서 주셔도 됩니다. 나눠 받아도 합쳐서 저장됩니다.',
    ''
  ];

  순서.forEach(function (키, i) {
    var 정보 = 출처표[키] || { 기관: '(출처 미지정)', 주제: '', URL: '', 메모: '' };
    조각.push('───────────────────────────────');
    조각.push('■ ' + (i + 1) + '. ' + 정보.기관 + (정보.주제 ? ' · ' + 정보.주제 : ''));
    if (정보.URL) 조각.push('   ' + 정보.URL);
    if (정보.메모) 조각.push('   (' + 정보.메모 + ')');
    조각.push('   이 페이지에서 확인할 항목 ' + 페이지별[키].length + '개');
    페이지별[키].forEach(function (하나) {
      조각.push('   - [' + 하나.주제 + '] ' + 하나.항목);
    });
    조각.push('');
  });
  ...
  조각.push('- 값에는 단위를 붙여 주세요. 예: `연 3.6%`, `최대 1,200만원`, `연소득 4,500만원 이하`');
  조각.push('- 기준일은 그 페이지에 적힌 시행일·갱신일을 쓰고, 없으면 오늘(' + 오늘 + ')로 하세요.');
  조각.push('- 항목 이름은 위에 적힌 그대로 쓰고 바꾸지 마세요.');
  조각.push('- 주제 칸도 대괄호 안에 적힌 그대로 써 주세요. 이 칸으로 어느 카드에 넣을지 정합니다.');
  조각.push('- 확인 못 한 항목은 그 줄을 아예 빼세요. 빈칸이나 "확인 필요"로 채우지 마세요.');
  조각.push('');
  조각.push('[출력 형식]');
  조각.push('표 하나만 답해 주세요. 인사말이나 설명은 붙이지 마세요.');
  조각.push('');
  조각.push('| 주제 | 항목 | 값 | 기준일 | 출처 |');
  조각.push('|---|---|---|---|---|');
  var 첫줄 = 페이지별[순서[0]][0];
  var 첫출처 = (출처표[순서[0]] || {}).URL || 'https://...';
  조각.push('| ' + 첫줄.주제 + ' | ' + 첫줄.항목 + ' | (확인한 값) | ' + 오늘 + ' | ' + 첫출처 + ' |');

  return {
    요청문: 조각.join('\n'),
    주제수: 주제들.length, 항목수: 총항목, 페이지수: 순서.length, 글수: 총글,
    주제들: 주제들.map(function (하나) {
      return { 이름: 하나.이름, 글수: 하나.글수, 항목: 하나.항목.length,
               채움: 하나.채움, 오래됨: 하나.오래됨 };
    }),
...
/**
 * 주제 칸이 들어 있는 표를 받아 주제별로 나눠 저장한다.
 *
...
function finwiz전체카드저장(답변) {
  ...
  function 주제찾기(글자) {
    var 열쇠 = finwiz소제목열쇠(글자);
    if (!열쇠) return '';
    for (var i = 0; i < 주제이름들.length; i++) {
      if (finwiz소제목열쇠(주제이름들[i]) === 열쇠) return 주제이름들[i];
    }
    // 부분 일치도 받아 준다 ('전세대출' → '전세대출 공통')
    for (var j = 0; j < 주제이름들.length; j++) {
      var 그것 = finwiz소제목열쇠(주제이름들[j]);
      if (그것.indexOf(열쇠) === 0 || 열쇠.indexOf(그것) === 0) return 주제이름들[j];
    }
    return '';
  }
  ...
  String(답변 || '').split('\n').forEach(function (줄) {
    ...
    if (칸.length < 3) return;
    if (/^주제$/.test(칸[0])) return;                      // 머리줄
    var 주제 = 주제찾기(칸[0].replace(/^\[|\]$/g, ''));
    if (!주제) { if (칸[0]) 모르는주제[칸[0]] = true; return; }
    if (!칸[1] || !칸[2]) return;
    읽은줄++;
    if (!주제별[주제]) 주제별[주제] = [];
    // 주제 칸을 떼어내고 기존 저장 함수가 아는 모양으로 되돌린다
    주제별[주제].push('| ' + 칸[1] + ' | ' + 칸[2] + ' | ' +
                      (칸[3] || '') + ' | ' + (칸[4] || '') + ' |');
  });
  ...
  if (!읽은줄) {
    var 안내 = ['표를 찾지 못했습니다.',
                '`| 주제 | 항목 | 값 | 기준일 | 출처 |` 모양의 표를 그대로 복사해서 붙여넣어 주세요.'];
    var 모름 = [];
    for (var 하나 in 모르는주제) 모름.push(하나);
    if (모름.length) {
      안내.push('주제 칸을 못 알아본 것: ' + 모름.slice(0, 5).join(', '));
      안내.push('주제 이름은 요청문의 대괄호 안에 적힌 그대로 써야 합니다.');
    }
    return { 성공: false, 문제들: 안내 };
  }

  var 결과들 = [], 실패 = [];
  for (var 주제이름 in 주제별) {
    try {
      var 한것 = finwiz사실카드저장(주제이름, 주제별[주제이름].join('\n'));
      if (한것.성공) 결과들.push({ 주제: 주제이름, 개수: 한것.개수, 아직: 한것.아직 });
      else 실패.push(주제이름 + ': ' + (한것.문제들 || []).join(' '));
    } catch (오류) {
      실패.push(주제이름 + ': ' + 오류.message);
    }
  }

  결과들.sort(function (가, 나) { return 나.개수 - 가.개수; });
  var 모름2 = [];
  for (var 둘 in 모르는주제) 모름2.push(둘);
  return { 성공: true, 읽은줄: 읽은줄, 결과들: 결과들,
           실패: 실패, 모르는주제: 모름2.slice(0, 5) };
}
...
function finwiz티스토리목록(도메인) {
  ...
  for (var i = 0; i < 주소들.length; i += 20) {
    ...
    답들.forEach(function (답, k) {
      ...
      if (!제목찾기) return;
      var 후보 = finwiz제목후보(제목찾기[1]);
      if (후보.length) {
        결과.글들.push({ 제목: 후보[0], 후보: 후보, URL: 묶음[k] });
      }
    });
  }
...
/**
 * 브라우저 제목에서 진짜 글 제목이 될 만한 후보를 모두 만든다.
 *
 * 스킨마다 모양이 다르다. '제목 :: 블로그이름' 도 있고
 * '블로그이름 :: 제목' 도 있고 '제목 - 블로그이름' 도 있다.
 * 어느 쪽인지 알 수 없으니 조각을 다 만들어 놓고 하나씩 맞춰 본다.
 *
 * 로드맵 제목에 들어 있는 전각 세로줄(｜)로는 자르지 않는다.
 * 그것으로 자르면 제목 자체가 반토막 난다.
 */
function finwiz제목후보(글자) {
  var 제목 = finwiz제목다듬기(글자);
  if (!제목) return [];
  var 후보 = [제목];
  [' :: ', '::', ' - ', ' — ', ' – ', ' > ', ' » '].forEach(function (칼) {
    var 자리 = 제목.indexOf(칼);
    while (자리 > 0) {
      var 앞 = 제목.substring(0, 자리).trim();
      var 뒤 = 제목.substring(자리 + 칼.length).trim();
      if (앞) 후보.push(앞);
      if (뒤) 후보.push(뒤);
      자리 = 제목.indexOf(칼, 자리 + 칼.length);
    }
  });
  return 후보.filter(function (값, i) {
    return 값.length >= 6 && 후보.indexOf(값) === i;
  });
}
...
function finwiz블로그목록쓰기메뉴() {
  ...
  try {
    ...
    ui.alert('finwiz_블로그글목록 탭에 적었습니다.\n\n' +
             ...
             '제목이 정말 다르면, 그 줄의 주소를 ' + finwiz순서탭 + ' 탭의\n' +
...
function finwiz주소반영() {
  ...
  var 진단 = {
    ...
    블로그에서찾음: 0, 블로그실패: '', 아직없는글: 0, 블로그제목예: [],
    쓴글수: 0, 쓴글중주소없음: 0, 주소없는쓴글예: []
  };
  ...
  if (아직없음.length) {
    ...
    if (도메인) {
      ...
      진단.블로그주소수 = 목록.주소수;
      진단.블로그실패 = 목록.실패;
      진단.블로그제목예 = 목록.글들.slice(0, 4).map(function (한개) { return 한개.제목; });
  ...
  // 진짜 봐야 할 숫자는 이것이다. 아직 안 쓴 글은 주소가 없는 게 당연하다.
  // 글을 써 놓고도 주소를 못 찾은 것만 문제다.
  진단.쓴글수 = 0;
  진단.쓴글중주소없음 = 0;
  진단.주소없는쓴글예 = [];
  순서목록.forEach(function (한줄) {
    if (한줄.작성상태 !== '작성완료' && 한줄.작성상태 !== '발행완료') return;
    진단.쓴글수++;
    var 글 = 링크맵[한줄.ID];
    if (글 && 글.URL) return;
    if (찾은주소[한줄.ID]) return;
    진단.쓴글중주소없음++;
    if (진단.주소없는쓴글예.length < 5) {
      진단.주소없는쓴글예.push(한줄.순번 + '번 ' + 한줄.제목);
    }
  });
...
function finwiz주소반영메뉴() {
  ...
  try {
    ...
    if (결과.반영) {
      ...
      if (진 && 진.블로그도메인) {
        말 += '\n\n[' + 진.블로그도메인 + ' 에서 읽어 본 것]\n';
        말 += '  글 주소 ' + 진.블로그주소수 + '개를 찾았고, 그중 ' +
              진.블로그글수 + '개의 제목을 읽었습니다.\n';
        말 += '  로드맵 제목과 맞은 것: ' + (진.블로그에서찾음 || 0) + '개\n';
        if (진.블로그실패) 말 += '  → ' + 진.블로그실패 + '\n';
        if (진.블로그제목예 && 진.블로그제목예.length) {
          말 += '\n  블로그에서 읽은 제목 (모양 확인용):\n   · ' +
                진.블로그제목예.join('\n   · ') + '\n';
        }
      }
      if (진) {
        말 += '\n쓴 글 ' + 진.쓴글수 + '편 중 아직 주소를 못 찾은 글 ' +
              진.쓴글중주소없음 + '편.';
        if (진.쓴글중주소없음 === 0) 말 += ' 다 채워졌습니다.';
      }
      ui.alert(말);
      return;
    }
    ...
    글 += '● 지금까지 쓴 글            : ' + 진.쓴글수 + '편\n';
    글 += '● 그중 주소를 못 찾은 글    : ' + 진.쓴글중주소없음 + '편  ← 이 숫자만 보세요\n';
    글 += '● 아직 안 쓴 글까지 합치면  : ' + 진.아직없는글 + '편 (안 쓴 글은 주소가 없는 게 정상)\n\n';
    if (진.쓴글중주소없음 === 0) {
      글 += '【결론】\n';
      글 += '쓴 글은 주소가 다 채워져 있습니다. 더 할 일이 없습니다.\n';
      글 += '남은 ' + 진.아직없는글 + '편은 아직 안 쓴 글이라 주소가 없는 것이 맞습니다.';
    ...
    // 가장 흔한 원인부터 짚는다. 임시저장 글은 블로그에 없으므로
    // 아무리 잘 찾아도 나올 수가 없다.
    if (진.쓴글중주소없음 > 0 && 진.블로그글수 > 0) {
      글 += '【가장 먼저 볼 것 — 그 글들이 정말 공개됐나요?】\n';
      글 += '티스토리 관리 → 글 관리 를 열어 위 글들이 "임시저장" 에\n';
      글 += '남아 있는지 보세요. 임시저장 글은 블로그에 없기 때문에\n';
      글 += '아무리 찾아도 나오지 않습니다.\n';
      글 += '공개로 발행하시면 저절로 풀립니다.\n\n';
      글 += '(공개인데도 안 나온다면 발행 직후일 수 있습니다.\n';
      글 += ' 티스토리가 글 목록에 올리는 데 몇십 분 걸립니다.)\n\n';
      글 += '그래도 안 되면 아래 4번 방법을 쓰세요.\n';
    } else if (진.블로그도메인 && 진.블로그글수 === 0) {
      글 += '【무엇이 문제인가】\n';
    ...
    글 += '\n【4번 · 직접 붙여넣기 — 가장 확실합니다】\n';
    글 += finwiz순서탭 + ' 탭의 "발행URL" 칸(O열)에 글 주소를 붙여넣고\n';
