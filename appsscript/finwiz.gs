...
function finwiz주소반영() {
  ...
  // 그것도 받아들인다. RSS 로 못 찾는 옛 글은 이 길로 살릴 수 있다.
  순서목록.forEach(function (한줄) {
...
function finwiz주소반영메뉴() {
  ...
  try {
    ...
    if (결과.반영) {
      ...
      if (진 && 진.손으로적음) {
        말 += '\n\n그중 ' + 진.손으로적음 + '개는 사장님이 발행URL 칸에 직접 적으신 주소입니다.';
      }
      ui.alert(말);
      return;
    }
    ...
    var 글 = '새로 적을 주소가 없습니다. 어디서 막혔는지 살펴봤습니다.\n\n';
    글 += '● 오늘작성 + 발행완료 탭에서 본 글 : ' + 진.줄수 + '줄\n';
    글 += '● 그중 티스토리 주소가 적힌 글    : ' + 진.주소있는줄 + '줄\n';
    글 += '● 주소 칸이 비어 있는 글          : ' + 진.주소빈줄 + '줄\n';
    글 += '● 로드맵 제목과 맞은 글           : ' + 진.제목맞은줄 + '줄\n';
    글 += '● 이미 주소가 적혀 있어 건너뛴 글 : ' + 진.이미있음 + '개\n';
    if (진.탭없음.length) 글 += '● 읽지 못한 탭 : ' + 진.탭없음.join(', ') + '\n';
    글 += '\n';

    if (진.주소있는줄 === 0 && 진.주소빈줄 > 0) {
      글 += '【무엇이 문제인가】\n';
      글 += '티스토리 주소 칸이 전부 비어 있습니다. 이 칸은 "🔎 발행됐는지 확인" 이\n';
      글 += '블로그의 글 목록(RSS)을 읽어서 채웁니다.\n\n';
      글 += '【해 볼 것 — 위에서부터】\n';
      글 += '1. "🔎 발행됐는지 확인" 을 눌러 보세요.\n';
      글 += '2. 발행한 지 얼마 안 됐으면 RSS 에 아직 안 실립니다. 10분쯤 뒤 다시.\n';
      글 += '3. 브라우저로 finwiz.tistory.com/rss 를 열어 보세요.\n';
      글 += '   글이 20~30개만 보인다면, 그보다 오래된 글은 RSS 로는 못 찾습니다.\n';
      글 += '   그럴 때는 4번으로 하시면 됩니다.\n';
      글 += '4. 최종작성순서 탭의 "발행URL" 칸에 주소를 직접 붙여넣고,\n';
      글 += '   이 메뉴를 다시 누르세요. 손으로 적은 주소도 그대로 씁니다.\n\n';
      글 += '주소 칸이 비어 있는 글 (예): \n   ' + 진.주소빈예.join('\n   ');
    } else if (진.주소있는줄 > 0 && 진.제목맞은줄 === 0) {
      글 += '【무엇이 문제인가】\n';
      글 += '주소는 있는데 제목이 로드맵 제목과 달라서 짝을 못 찾습니다.\n';
      글 += '봇이 제목을 고쳐 쓴 경우입니다.\n\n';
      글 += '【해 볼 것】\n';
      글 += '오늘작성 탭의 제목을 최종작성순서 탭의 제목과 똑같이 고치고\n';
      글 += '이 메뉴를 다시 누르세요.\n\n';
      글 += '짝을 못 찾은 제목 (예): \n   ' + 진.제목틀린예.join('\n   ');
    } else if (진.이미있음 > 0) {
      글 += '【무엇이 문제인가】\n';
      글 += '찾은 글은 이미 주소가 다 적혀 있습니다. 더 적을 것이 없습니다.\n';
      글 += '이 상태가 정상입니다.';
    } else {
      글 += '【해 볼 것】\n';
      글 += '1. "🔎 발행됐는지 확인" 을 먼저 눌러 보세요.\n';
      글 += '2. 그래도 안 되면 최종작성순서 탭의 "발행URL" 칸에\n';
      글 += '   주소를 직접 붙여넣고 이 메뉴를 다시 누르세요.';
    }
